...
\author[d2]{James E. Bevins, Larry W. Burggraf, Buckley E. O'Day}
...
%
% Comment addressed or should I add more?  Feel free to modify
...
Instead of using only the full energy peak (FEP), the DRC is formed by adding all counts above 200keV to increase the source direction identification's efficiency.  
...
Considering the vector shifts, the maximum MAC number, which corresponds to the most similar pair of DRCs is given in Eqns.~\ref{eq:maxmac}.
...
The eigenvector approach solves a mass normalized eigenvalue problem.  The binary method uses patterns of ones and zeros to represent the geometry.
...
The average MAC number, $A$, is given in Eqn.~\ref{eq:avgmac} and provides information about the mean linear dependence level.
\begin{equation}
...
Note that the average, minimum, and maximum DRC values for a given $\phi$ remain the same for different initial $\theta$ values.  
...
% I was wrong about this criteria.  It's not the averge normalized counts over theta and phi positions.  It is the average normalized counts for a given cell (# of cells = n * m)
...
is minimized for every combination of numbers $\textbf{a}$ and $\textbf{b}$ including their the corresponding cyclically shifted versions.
...
Thus, the total number of terms for $p=3$ and a given $n$ is $t=t_1+t_2$.  
